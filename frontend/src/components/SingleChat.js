...
const SingleChat = ({ fetchAgain, setFetchAgain }) => {
	...
	const fetchMessages = async () => {
		...
		try {
			...
			console.log("after Fetching message", data);
			setMessages(data);
			setLoading(false);
			console.log("the selected chat", selectedChat);
	...
	useEffect(() => {
		socket.on("message receieved", (messageReceieved) => {
			...
			if (
				...
				if (!notification.includes(messageReceieved)) {
					setNotification([...notification, messageReceieved]);
					setFetchAgain(!fetchAgain);
				}
			}
			setMessages([...messages, messageReceieved]);
		});
	});
